...
class PositionalEncoding(nn.Module):
    def __init__(self, embedding_dim: int, seq_len: int):
        ...
        self.pe = torch.zeros((self.seq_len, self.embedding_dim), device= self.device)
        self.pe.requires_grad = False

        position = torch.arange(0, self.seq_len, dtype= torch.float, device= self.device).unsqueeze(1) # [seq_len, 1]
        div_term = torch.arange(0, self.embedding_dim, 2, device= self.device).float()
